import { X, Heart, Share2, MapPin, Star, MessageCircle, ChevronLeft, ChevronRight } from "lucide-react";
...
import { mockSellers, mockPins } from "@/data/mockData";
...
const ListingDrawer = ({ pin, isOpen, onClose }: ListingDrawerProps) => {
  ...
  const similarPins = mockPins.filter((p) => p.id !== pin.id && p.category === pin.category).slice(0, 4);
  
  // Mock multiple images
  const images = [pin.image, pin.image, pin.image];
  ...
  return (
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent side="right" className="w-full sm:w-[540px] sm:max-w-[540px] p-0 overflow-y-auto">
        {/* Close Button */}
        <button
          onClick={onClose}
        ...
        <div className="relative aspect-square bg-muted">
          ...
          <div className="absolute top-4 right-4 flex gap-2">
            ...
            <button className="w-10 h-10 rounded-full bg-card/90 backdrop-blur-sm flex items-center justify-center text-foreground hover:bg-card transition-colors">
        ...
        <div className="p-6 space-y-6">
          ...
          {similarPins.length > 0 && (
            <div className="space-y-4 pt-4 border-t border-border">
              <h3 className="font-display font-medium text-foreground">Similar Items</h3>
                  ...
                  >
                    ...
                    <div className="p-2 space-y-1">
                      ...
                      <p className="text-sm font-semibold text-primary">
                        {item.isTradeOnly ? "Trade" : `$${item.price}`}
                      </p>
