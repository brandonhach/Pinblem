...
import { mockPins, categories, type Pin } from "@/data/mockData";
...
const Search = () => {
  ...
  const locationOptions = useMemo(() => {
    const locs = [...new Set(mockPins.map((p) => p.location))].sort();
    return ["all", ...locs];
  }, []);
  ...
  const filteredPins = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      result = result.filter(
        (pin) =>
          ...
          pin.seller.toLowerCase().includes(query) ||
    ...
    if (selectedListingType !== "all") {
      if (selectedListingType === "trade") {
        result = result.filter((pin) => pin.isTradeOnly);
    ...
    result = result.filter(
      (pin) => pin.isTradeOnly || (pin.price >= priceRange[0] && pin.price <= priceRange[1])
    );

    if (selectedConditions.length > 0) {
      result = result.filter((pin) => selectedConditions.includes(pin.condition));
    }
    ...
    return result;
  }, [searchQuery, selectedCategory, selectedListingType, priceRange, sortBy, selectedConditions, selectedLocation]);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="sticky top-16 z-40 bg-background border-b border-border px-4 py-3">
        <div className="container max-w-4xl">
          ...
          <div className="flex items-center justify-between mt-3">
            <span className="text-sm text-muted-foreground">{filteredPins.length} results</span>
            <select
              value={sortBy}
            ...
            <div>
              ...
              <div className="flex gap-2">
                {[
                  ...
                  <Button
                    key={type.value}
                    variant={selectedListingType === type.value ? "default" : "outline"}
                    size="sm"
          ...
          {filteredPins.length > 0 ? (
            <>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 items-stretch auto-rows-fr">
                {visiblePins.map((pin) => (
                  <ListingCard key={pin.id} pin={pin} onClick={() => handleCardClick(pin)} />
                ))}

                {isLoadingMore &&
                  Array.from({ length: 4 }).map((_, i) => (
                    <div key={`sk-${i}`} className="card-tactile overflow-hidden h-full flex flex-col">
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-semibold text-foreground">No pins found</h3>
              <p className="text-sm text-muted-foreground mt-1">Try adjusting your filters or search terms</p>
              <Button size="sm" onClick={clearFilters} className="mt-4">
                Clear Filters
              </Button>
            </div>
          )}
      ...
      <ListingDrawer pin={selectedPin} isOpen={isDrawerOpen} onClose={() => setIsDrawerOpen(false)} />
    </div>
  );
};
